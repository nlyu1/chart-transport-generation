...
$
The autoencoder defines a *chart* subject to the following constraints: roundtrip is identity on the prior and data distributions
$(g_theta compose f_phi)("data") = "data"$ and
$(f_phi compose g_theta)("prior") = "prior"$
We know that the final data latent we target is the prior distribution $nu = cal(N)(0, 1)$; this is a closed-form target with (theoretically) support on all of $RR^n$, so it seems natural to score-match in latent space. We use #blue[$rho^+$ for the data distribution, $rho^- = g_(theta\#) (nu)$ for the model distribution, and $x tilde.op rho^plus.minus$ for samples]. We also write #blue[$sigma^plus.minus = f_(phi\#) (rho^plus.minus)$ for the latent distributions, and $y tilde.op sigma^plus.minus$ for latents.]
$
...
We use #blue[a separate score network $s_psi$ to learn the score of the data latent $sigma^+ = f_(phi \#) rho^+$]. The score difference $delta(y) = EE_t [nabla log sigma^+_t (y) - nabla log sigma^-_t (y)]$ provides a field on the latent space which drifts the model latent towards data latent. We update the autoencoders to capture the drift.
...
1. Encoder-decoder bijectivity: $(g_theta compose f_phi)(nu) = nu quad (f_phi compose g_theta )(rho^+) = rho^+$.
2. Score network correctly estimates the score: $h_psi (y_t) approx nabla log sigma^-_t (y_t)$.
3. Chart transport according to the score field $delta(y) = EE_t [nabla log sigma^+_t (y) - nabla log sigma^-_t (y)]$
$
  & EE_(x tilde.op rho^+, y tilde.op (sigma^+_t | x)) ||g_theta (y + eta dot delta(y)) - x||^2 \
  & EE_(x tilde.op rho^+, y tilde.op (sigma^+_t | x)) ||f_phi (x) - (y + eta dot delta(y))||^2
$
