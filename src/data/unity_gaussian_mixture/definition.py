...
from src.data.base import DatasetConfig, GenerativeBatch, GenerativeData
...
class UnityGaussianMixtureData(GenerativeData):
    x_2d: Float[Tensor, "batch plane_dim"]
    mode_ids: Int[Tensor, "batch"]


@dataclass(kw_only=True, frozen=True)
class UnityGaussianMixtureBatch(GenerativeBatch[UnityGaussianMixtureData]):
    x_hd: Float[Tensor, "batch ambient_dim"]
    x_2d: Float[Tensor, "batch plane_dim"]
    mode_ids: Int[Tensor, "batch"]

    def data(self) -> UnityGaussianMixtureData:
        return UnityGaussianMixtureData(
            x=self.x_hd,
            x_2d=self.x_2d,
...
def _collate_unity_gaussian_mixture_batch(
...
class UnityGaussianMixtureDatasetConfig(DatasetConfig):
    ...
    def collate_batch(
        ...
        return _collate_unity_gaussian_mixture_batch(samples)
